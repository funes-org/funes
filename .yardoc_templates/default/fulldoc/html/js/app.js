...
window.addEventListener(
  ...
  async (e) => {
    if (e.data.action === "navigate") {
      // Resolve URL relative to iframe location to handle versioned paths correctly
      ...
      // Detect if we're in development (yard server on localhost) or production (static files)
      const isDevelopment = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';

      // For yard server (development), use the original relative URL
      // For static files (production with versioned paths), use the resolved URL
      const fetchUrl = isDevelopment ? e.data.url : resolvedUrl.href;
      ...
      const hash = decodeURIComponent(resolvedUrl.hash ?? "");
      if (hash) {
        document.getElementById(hash.substring(1)).scrollIntoView();
      }
      // Use resolved pathname for versioned URLs, original URL for yard server
      const historyUrl = isDevelopment ? e.data.url : resolvedUrl.pathname;
