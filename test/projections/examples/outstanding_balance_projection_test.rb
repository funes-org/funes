...
class Examples::OutstandingBalanceProjectionTest < ActiveSupport::TestCase
  ...
  describe "events handling" do
    describe "debt issued" do
      it "sets the event value as initial outstanding balance" do
        assert_equal 10, interpret_event_based_on(Examples::OutstandingBalanceProjection, Debt::Issued.new(value: 10), nil)
      end
    end

    describe "debt paid" do
      it "reduces the outstanding balance by payment value and discount amount (sum of both)" do
        assert_equal 10, interpret_event_based_on(Examples::OutstandingBalanceProjection,
      ...
      it "raises an error when payment processing would result in negative balance" do
        assert_raise Funes::LedToInvalidState, "The outstanding balance can't be negative" do
          interpret_event_based_on(Examples::OutstandingBalanceProjection, Debt::Paid.new(value: 5, discount: 5), 5)
        end
      end
    end
    ...
    describe "debt adjusted by index" do
      it "increases outstanding balance by the specified positive rate" do
        assert_equal 110, interpret_event_based_on(Examples::OutstandingBalanceProjection,
                                                   Debt::AdjustedByIndex.new(rate: 0.1), 100)
      end

      it "decreases outstanding balance when applying negative rate" do
        assert_equal 90, interpret_event_based_on(Examples::OutstandingBalanceProjection,
                                                  Debt::AdjustedByIndex.new(rate: -0.1), 100)
      end

      it "keeps outstanding balance unchanged when rate is zero" do
        assert_equal 100, interpret_event_based_on(Examples::OutstandingBalanceProjection,
  ...
  describe "the handling for an entire debt events stream" do
    ...
    it "processes all events and results in zero balance after the last payment" do
      assert_equal 0, Examples::OutstandingBalanceProjection.process_events(events)
    end

    it "partially processes the events and infers the proper outstanding balance before the last payment" do
      assert_equal 51.5, Examples::OutstandingBalanceProjection.process_events(events.take(3))
    end
  end
end
