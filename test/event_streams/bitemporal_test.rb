...
class BitemporalTest < ActiveSupport::TestCase
  ...
  describe "projected_with with no events to project" do
    it "raises ActiveRecord::RecordNotFound when the stream has no events at all" do
      ...
      assert_raises ActiveRecord::RecordNotFound do
        stream_without_actual_time.for(idx).projected_with(salary_projection)
      end
    end

    it "raises ActiveRecord::RecordNotFound when at: filters out every event" do
      idx = "salary-at-excludes-all-#{SecureRandom.uuid}"
      stream = stream_without_actual_time.for(idx)
      stream.append(SalarySet.new(amount: 6000), at: Time.new(2025, 4, 1))
    ...
    it "raises ActiveRecord::RecordNotFound when as_of: filters out every event" do
      idx = "salary-as-of-excludes-all-#{SecureRandom.uuid}"
      ...
      assert_raises ActiveRecord::RecordNotFound do
        stream_without_actual_time.for(idx).projected_with(salary_projection, as_of: Time.new(2025, 1, 1))
      end
    end

    it "exposes idx on the raised error so logs and trackers see the identifier" do
      idx = "salary-error-attrs-#{SecureRandom.uuid}"
    ...
    it "logs an info line with stream, idx, projection, and filter context before raising" do
      idx = "salary-log-#{SecureRandom.uuid}"
      io = StringIO.new
