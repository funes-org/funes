...
- `refers_to` macro for events and materialization models — declare that an
  event, or a projection's materialization model, references a model
  (e.g. `refers_to :customer`) and read the record back at interpretation
  time. Only the record's id is stored in the payload; the reader loads the
  record lazily. Supports `class_name:`, `foreign_key:`, and `required:`
