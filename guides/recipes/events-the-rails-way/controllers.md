...
- **`redirect_to debt_path(event_stream)`** — event streams plug into Rails URL helpers just like an `ActiveRecord` model. Pass one to `redirect_to`, `link_to`, or any `<resource>_path` helper. Rails resolves the identifier on its own, with no extra wiring.
