...
- [Rendering 404s from empty streams](/recipes/events-the-rails-way/show-actions/) — `projected_with` returns the materialization model on a hit and raises `ActiveRecord::RecordNotFound` on a miss. Rails 404s automatically, with no rescue code.
